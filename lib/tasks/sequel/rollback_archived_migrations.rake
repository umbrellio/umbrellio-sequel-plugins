...
namespace :sequel do
  ...
  task rollback_archived_migrations: :environment do
    ...
    migrations_to_rollback = applied_migrations.select { |m| m.in?(candidate_migrations) }.sort.reverse
    ...
    migrations_to_rollback.each { |migration| migrator.undo(migration) }
  end
end
